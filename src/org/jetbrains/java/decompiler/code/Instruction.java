...
public class Instruction implements CodeConstants {
  public static Instruction create(int opcode, boolean wide, int group, int bytecodeVersion,int offset, int[] operands) {
    if (opcode >= opc_ifeq && opcode <= opc_if_acmpne ||
      ...
      return new JumpInstruction(opcode, group, wide, bytecodeVersion, offset,operands);
    }
    else if (opcode == opc_tableswitch || opcode == opc_lookupswitch) {
      return new SwitchInstruction(opcode, group, wide, bytecodeVersion, offset,operands);
    }
    else {
      return new Instruction(opcode, group, wide, bytecodeVersion, offset,operands);
    }
  }
  ...
  public Instruction(int opcode, int group, boolean wide, int bytecodeVersion,int offset, int[] operands) {
    this.opcode = opcode;
    this.group = group;
    this.wide = wide;
  ...
  public Instruction clone() {
    return create(opcode, wide, group, bytecodeVersion,this.offset, operands == null ? null : operands.clone());
  }
}