...
public class FinallyProcessor {
    ...
    for (BasicBlock block : setTry) {
      for (BasicBlock dest : block.getSuccessors()) {
        ...
        if (dest != graph.getLast() && !setCopy.contains(dest)) {
          ...
          seq.addInstruction(Instruction.create(CodeConstants.opc_bipush, false, CodeConstants.GROUP_GENERAL, bytecodeVersion, -1,new int[]{0}), -1);
          seq.addInstruction(Instruction.create(CodeConstants.opc_istore, false, CodeConstants.GROUP_GENERAL, bytecodeVersion, -1,new int[]{var}), -1);
  ...
  private static BasicBlock createHeadBlock(ControlFlowGraph graph, int value, int var, int bytecodeVersion) {
    SimpleInstructionSequence seq = new SimpleInstructionSequence();
    seq.addInstruction(Instruction.create(CodeConstants.opc_bipush, false, CodeConstants.GROUP_GENERAL, bytecodeVersion,-1, new int[]{value}), -1);
    seq.addInstruction(Instruction.create(CodeConstants.opc_istore, false, CodeConstants.GROUP_GENERAL, bytecodeVersion, -1,new int[]{var}), -1);
