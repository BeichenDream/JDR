  ...
  public static void insertDummyExceptionHandlerBlocks(ControlFlowGraph graph, int bytecode_version) {
    ...
    for (Entry<BasicBlock, Set<ExceptionRangeCFG>> ent : mapRanges.entrySet()) {
      ...
      for (ExceptionRangeCFG range : ranges) {
        ...
        seq.addInstruction(Instruction.create(CodeConstants.opc_bipush, false, CodeConstants.GROUP_GENERAL, bytecode_version,-1, new int[]{0}), -1);
        seq.addInstruction(Instruction.create(CodeConstants.opc_pop, false, CodeConstants.GROUP_GENERAL, bytecode_version,-1, null), -1);
